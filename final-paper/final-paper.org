...
OAK has many opportunies for future work.

*Dynamic index construction*. 
Right now, OAK constructs indices only once and before queries are dispatched.
In a production system, it may be advantageous to contruct indices while queries are received to increase throughput.
